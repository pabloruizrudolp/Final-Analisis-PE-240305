...
t1.1<-HacerTabla("preclampsi",varsnewlursp, 10, covsMEQ, newstrand)
t1.2<-HacerTabla("preclampsi",varsnewlursp, 10, covsMEQ, newfullstrand)
t1.3<-HacerTabla("preclampsi",varsnewlursp, 10, covsMEQ, newcoh)
t1.4<-HacerTabla("preclampsi",varsnewlursp, 10, covsMEQ, newfullcoh)

# PROBAR VARSPM (NEWLUR vs MEQ) covs ANG
t2.1<-HacerTabla("preclampsi",varsnewlursp, 10, covsAngApe, newstrand)
t2.2<-HacerTabla("preclampsi",varsnewlursp, 10, covsAngApe, newfullstrand)
t2.3<-HacerTabla("preclampsi",varsnewlursp, 10, covsAngApe, newcoh)
t2.4<-HacerTabla("preclampsi",varsnewlursp, 10, covsAngApe, newfullcoh)
...
my_doc<-my_doc %>%
  body_add_par("Tabla 1.1 PE Covs:MEQ PM:NEWDATA (set new strand)", style = "Normal") %>%
  body_add_flextable(t1.1)  %>%
  body_add_par("Tabla 1.2 PE Covs:MEQ PM:NEWDATA (set new full strand)", style = "Normal") %>%
  body_add_flextable(t1.2) %>%
  body_add_par("Tabla 1.3 PE Covs:MEQ PM:NEWDATA (set new coh)", style = "Normal") %>%
  body_add_flextable(t1.3) %>%
  body_add_par("Tabla 1.4 PE Covs:MEQ PM:NEWDATA (set new full coh)", style = "Normal") %>%
  body_add_flextable(t1.4) %>%

  body_add_break() %>%

  body_add_par("Tabla 2.1 PE Covs:ANG PM:NEWDATA (set new strand)", style = "Normal") %>%
  body_add_flextable(t2.1)  %>%
  body_add_par("Tabla 2.2 PE Covs:ANG PM:NEWDATA (set new full strand)", style = "Normal") %>%
  body_add_flextable(t2.2) %>%
  body_add_par("Tabla 2.3 PE Covs:ANG PM:NEWDATA (set new coh)", style = "Normal") %>%
  body_add_flextable(t2.3) %>%
  body_add_par("Tabla 2.4 PE Covs:ANG PM:NEWDATA (set new full coh)", style = "Normal") %>%
  body_add_flextable(t2.4) %>%
  
print(my_doc, target="Tables/Tabla 4.2 PE compare new_newfull 230525.docx")
